...
export function useDataSources() {
  ...
  const dataSources = computed(() => {
    ...
    return dataSourceStore.dataSources.map(dataSource => {{
      let status
      let dataLoader = dataLoaderStore.dataLoaders.filter(dl => dl.id === dataSource.dataLoaderId)[0]
      let statusTip = null
      let now = new Date()
      ...
      if (lastSynced == null) {
        ...
        dataSource['lastSyncSuccessful'] && nextSync &&
        nextSync >= now
      ) {
        status = 'ok'
      ...
      let datastreams = Object.values(datastreamStore.datastreams).flat(1).filter(
  ...
  async function togglePaused() {
    ...
    let dataSource = structuredClone(toRaw(dataSourceStore.getById(selectedDataSource.value)))
  ...
  async function reloadDataSources() {
    ...
    await datastreamStore.fetchDatastreams(true)
  }
